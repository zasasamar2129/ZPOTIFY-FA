...
class SpotifyDownloader:
    ...
    async def send_track_info(client, event, link_info):
        ...
        SpotifyInfoButtons = [
            ...
            [Button.inline("اطلاعات هنرمند ", data=f"spotify/artist/{link_info['track_id']}")],
            [Button.inline("متن آهنگ", data=f"spotify/lyrics/{link_info['track_id']}")],
            [Button.url("در Spotify گوش کنید", url=link_info["track_url"]),
             Button.url("در یوتیوب گوش کنید", url=link_info['youtube_link']) if link_info[
                 'youtube_link'] else Button.inline("در یوتیوب گوش کنید", data=b"unavailable_feature")],
            [Button.inline("لغو", data=b"cancel")]
        ]

        caption = (
            f"**🎧 عنوان:** [{link_info['track_name']}]({link_info['track_url']})\n"
            f"**🎤 هنرمند:** [{link_info['artist_name']}]({link_info['artist_url']})\n"
            f"**💽 آلبوم:** [{link_info['album_name']}]({link_info['album_url']})\n"
            f"**🗓 سال انتشار:** {link_info['release_year']}\n"
            f"**❗️ محلی است:** {is_local}\n"
            f"**🌐 ISRC:** {link_info['isrc']}\n"
            f"**🔄 دانلود شده:** {await db.get_song_downloads(link_info['track_name'])} times\n\n"
            f"**لینک تصویر:** [Click here]({link_info['image_url']})\n"
            f"**شناسه آهنگ:** {link_info['track_id']}\n"
        )
